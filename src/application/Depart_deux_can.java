...
public class Depart_deux_can implements Operation {
    ...
    public Etat action(Etat etat) {
        ...
        int varLoc2 = (3 - etatCourant.getNbre_cannibales()) + 2;

        if (((etatCourant.isBateau() == true) && (2 <= etatCourant.getNbre_cannibales())) &&
                ((varLoc1 == 0) || (varLoc1 >= varLoc2)) && (((3-varLoc1)==0) || ((3-varLoc1) >= (3-varLoc2)))) {
            return new Etat_can_miss(etatCourant.getNbre_cannibales() - 2, etatCourant.getNbre_missionnaires(), false);
        } else {
            return null;
        }
    }
